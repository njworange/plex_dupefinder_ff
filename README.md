...
11. 여러 그룹을 처리하려면 `일괄 승인형 반자동 삭제 활성화`를 추가로 켜고 `스캔 1회당 전역 삭제 시도 한도`를 필요한 범위만큼 올립니다.
...
14. 처리된 그룹은 부분 스캔 성공 여부와 관계없이 잠기며, 다음 작업 전에 DupeFinder에서 다시 스캔해야 합니다.
