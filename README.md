...
Plex 연결정보는 별도로 저장하지 않고, 작업을 시작할 때마다 [`plex_mate`](https://github.com/flaskfarm/plex_mate)의 `base_url`, `base_token`, `base_machine`을 읽습니다. 기본 `plex` 방식은 PMS에 Media 삭제를 요청합니다. 명시적으로 선택하는 `quarantine` 방식은 PMS DELETE를 호출하지 않고 FlaskFarm에서 삭제 대상 영상과 안전하게 귀속된 외부 자막을 격리 폴더로 이동합니다. Plex SQLite DB를 직접 수정하지 않습니다.

> [!CAUTION]
> Plex 방식은 해당 Media 버전의 실제 Part 파일과 연관 파일을 삭제할 수 있습니다. 안전 격리는 실제 파일을 이동하므로 올바른 경로·권한과 백업이 필요합니다. 그룹별 처리와 일괄 승인형 처리는 모두 기본적으로 꺼져 있으며, 사용자 검토 없이 실행되는 완전 자동 처리는 제공하지 않습니다.
...
- 결과와 감사 이력에서 함께 격리된 자막 및 위험 예외 확인·필터
...
- `requests` 2.25 이상
- 안전 격리 사용 시 Plex 미디어 경로와 격리 경로를 FlaskFarm 프로세스가 직접 읽고 쓸 수 있는 실행환경
...
8. 외부 자막을 유지본과 분리해 안전하게 정리하려면 `파일 처리 방식`을 **안전 격리**로 선택하고 FlaskFarm 호스트 기준 절대 `안전 격리 루트`를 입력합니다. 기존 Plex 방식에서는 자막 자동 정리가 꺼져 있습니다.
9. 안전 격리는 PMS DB 반영과 사후 검증을 위해 `삭제 성공 후 Plex 부분 스캔 방식`을 Binary 또는 Web으로 반드시 선택해야 합니다. 미디어 파일과 격리 루트가 같은 파일시스템인지도 확인합니다.
10. 단건 처리라면 결과의 `개별 삭제`를 눌러 유지·처리 버전, 함께 격리될 자막과 위험하여 제외된 자막의 전체 경로·사유를 확인한 뒤 정확한 확인 문구를 입력합니다.
11. 기본 `스캔 1회당 전역 삭제 시도 한도`는 1입니다. 같은 스캔 결과에서 두 건 이상을 개별 또는 배치로 처리하려면 한도를 필요한 범위만큼 명시적으로 올립니다. 설정을 저장하면 새로 스캔하지 않고 현재 결과를 계속할 수 있습니다.
12. 결과 화면에서 `삭제 계획 만들기`를 누르고 각 그룹의 유지·처리 Media ID, 점수와 파일 경로를 모두 검토합니다.
13. 화면에 표시된 일괄 확인 문구를 정확히 한 번 입력하면 항목별 재검증과 파일 처리가 순차 실행됩니다.
14. 처리된 그룹은 부분 스캔 성공 여부와 관계없이 잠깁니다. 남은 시도 한도가 있으면 같은 스캔의 다른 미처리 그룹을 계속할 수 있지만, 처리한 그룹을 최신 상태로 다시 판정하려면 DupeFinder 중복 스캔을 새로 실행합니다.
...
- 기존 Plex 방식은 `DELETE {media_id}`, 안전 격리는 `QUARANTINE {media_id} SUBTITLES {count} {digest12}` 정확 확인 문구
...
DELETE 요청이 timeout되거나 연결 결과를 확정할 수 없으면 같은 요청을 자동 재시도하지 않습니다. Plex를 다시 조회해 결과를 확인하며, 확정할 수 없으면 감사 이력에 `unknown`으로 기록합니다. 결과가 `unknown`이어도 실제 삭제가 수행됐을 수 있으므로 스캔별 삭제 시도 상한을 하나 소비합니다. FlaskFarm이 작업 도중 재시작되면 `validating` 이력은 `blocked`, `deleting` 이력은 `unknown`으로 복구하고 해당 그룹을 수동 확인 상태로 잠급니다. 안전 격리의 `quarantining` 이력은 durable journal과 실제 원본·격리 경로를 기준으로 `recovery_required`/`unknown`으로 보수 복구하며 자동 이동·복구를 이어가지 않습니다.

`스캔 1회당 전역 삭제 시도 한도`의 보수적 기본값은 1입니다. 결과 화면은 현재 시도/한도와 남은 횟수를 표시하고, 한도가 소진되면 개별 사전확인과 배치 계획 생성을 차단합니다. 운영자가 **설정 > 삭제 안전장치**에서 한도를 명시적으로 올려 저장하면 새 스캔 없이 현재 스캔에 남은 한도가 즉시 반영됩니다. 업데이트는 기존에 저장한 한도를 자동으로 바꾸지 않습니다.

## 삭제 후 Plex 부분 스캔

`삭제 성공 후 Plex 부분 스캔 방식`의 호환 기본값은 `사용 안 함(none)`입니다. 기존 Plex 방식에서는 Plex DELETE가 완료되고 Media 사후 재조회로 삭제 결과가 정확히 확인된 경우에만 durable outbox에 비파괴 스캔 요청을 기록합니다. 안전 격리 방식에서는 PMS DB 반영과 유지본 검증에 부분 스캔이 필수이므로 `none`을 허용하지 않으며, 파일 격리 이동이 완료된 뒤에만 요청합니다. 파일 처리 결과가 실패·불명확·검증 실패이면 요청하지 않습니다.

Binary 또는 Web을 선택하면 파일 처리 전에 현재 PMS에서 library section, 타입과 Location을 다시 읽고 모든 대상 Part에 대한 정확한 스캔 경로를 먼저 만듭니다. 영화 폴더나 TV 쇼 루트를 하나라도 안전하게 계산할 수 없거나 경로가 section/삭제 허용 루트를 벗어나면 DELETE 또는 격리 이동을 시작하지 않습니다. 기존 Plex 방식의 사후 스캔 전달은 best-effort지만 그 대상 경로를 정확히 준비하는 것은 파일 처리의 필수 사전조건입니다. 안전 격리에서는 부분 스캔과 유지본 사후검증이 성공 확정의 일부입니다. 기존 Plex 방식에서만 이 추가 차단을 원하지 않을 때 기본값 `none`을 유지할 수 있습니다.
...
기존 Plex 방식에서 사후 스캔은 best-effort 후처리입니다. 일시적 실패는 outbox에서 최대 3회까지 제한적으로 지수 backoff 재시도할 수 있지만, Plex DELETE는 절대로 재시도하지 않습니다. 사후 스캔의 실패나 최종 포기는 이미 확인된 Plex 삭제 성공을 실패로 바꾸거나 삭제를 다시 실행하지 않습니다. 안전 격리 방식은 다릅니다. 영상·자막 이동 뒤 부분 스캔과 유지본 사후검증까지 통과해야 성공이며, 최종 실패하면 `unknown`/`manual_check_required`로 남겨 운영자가 격리 저널과 Plex 상태를 확인하도록 합니다. 또한 이 기능은 DupeFinder 중복 재검사를 실행하지 않고 `rescan_required` 잠금을 해제하지 않습니다. 처리한 그룹의 최신 중복 판정은 DupeFinder 스캔을 별도로 실행해야 반영됩니다. 다만 백엔드는 각 항목을 실행 직전 다시 검증하므로, 남은 시도 한도 안에서는 같은 스캔의 다른 미처리 그룹을 계속할 수 있습니다.
