...
`plex_dupefinder_ff`는 FlaskFarm에서 동작하는 Plex 중복 검사·점수화, **선택 버전 한 클릭 삭제**, **사용자가 시작하는 중복 자동 정리** 플러그인입니다.
...
> [!CAUTION]
> Plex Media DELETE는 해당 Media 버전의 실제 Part 파일과 연관 파일을 삭제할 수 있습니다. 안전 격리는 실제 파일을 이동합니다. 외부 자막 정리 방식도 영상을 PMS로 영구삭제하고 전용 자막을 정리하므로, 올바른 경로·권한과 별도 백업이 필요합니다. 이 방식의 FlaskFarm data 보호본은 PMS가 유지본 자막까지 지웠을 때 되돌리기 위한 트랜잭션용 임시본이며, 성공 후 사용자가 복구할 백업이나 격리본이 아닙니다. 선택 버전 삭제와 중복 자동 정리는 모두 기본적으로 꺼져 있으며, 자동 정리도 사용자가 결과 화면에서 직접 시작해야 합니다. 시작 뒤에는 최고 score 버전 하나만 남기고 안전하게 확정된 나머지 버전을 추가 확인 없이 순차 처리합니다. 최고 score가 동률이면 Plex Media ID가 가장 작은 버전 하나를 유지합니다.
...
- 안전 조건에 맞는 그룹에서 선택한 버전을 문구 입력·브라우저 확인창 없이 한 클릭 삭제
- 실행 직전 서버에서 Plex 상태와 최신 파일 계획을 다시 검증
- 라이브러리 전체 선택 및 전체 선택 해제
- 사용자가 시작하면 각 안전 그룹의 최고 score 버전 하나만 유지하고 나머지 버전을 상한 없이 순차 처리하는 중복 자동 정리
- 최고 score 동점은 Plex Media ID가 가장 작은 버전 하나를 유지하며, unsafe·공유 경로·모호한 파일 계획 등 자동 확정할 수 없는 그룹은 사유와 함께 제외
...
- 최근 스캔 결과와 Action·부분 스캔 작업 이력을 DB에서 강제 정리할 수 있으며, ID tombstone과 복구 journal은 보존하고 파일/PMS는 호출하지 않음
...
7. 백업을 확인한 뒤 `선택 버전 삭제 활성화`를 켭니다. 전체 자동 정리를 원하면 `중복 자동 정리 활성화`도 켭니다.
...
10. 한 버전만 처리하려면 결과에서 삭제할 Media를 선택하고 `선택 버전 삭제`를 누릅니다. 별도의 확인 문구 입력이나 브라우저 확인창 없이, 서버가 최신 미리보기·CSRF·일회용 nonce·plan digest를 검증한 뒤 실행합니다.
11. 전체 정리를 원하면 결과 화면에서 `최고 점수만 남기고 자동 정리 시작`을 누릅니다. 각 안전 그룹의 최고 score 버전 하나를 유지하고 나머지 버전을 순차 처리합니다. 최고 score가 동률이면 Plex Media ID가 가장 작은 버전 하나를 유지하며, 포함·제외 그룹과 제외 사유를 진행 화면에서 확인할 수 있습니다.
12. 처리된 그룹은 부분 스캔 성공 여부와 관계없이 잠깁니다. 같은 그룹에서 하나가 실패하면 그 그룹의 남은 하위 버전만 건너뛰고, 다른 안전 그룹은 계속 처리합니다. 처리한 그룹을 최신 상태로 다시 판정하려면 DupeFinder 중복 스캔을 새로 실행합니다.

## 중복 자동 정리

이 기능은 스케줄이나 스캔 완료를 계기로 저절로 시작되지 않습니다. 사용자가 완료된 스캔의 결과 화면에서 `최고 점수만 남기고 자동 정리 시작`을 누르면 최신 계획을 만들고 곧바로 실행합니다. 문구 입력이나 별도의 브라우저 확인창은 없지만, 서버는 짧게 유효한 일회용 nonce, CSRF, 최신 plan digest와 전역 삭제 lease를 계속 검증합니다.

계획에는 다음 조건을 모두 만족하는 그룹만 포함됩니다.

- 완료된 동일 스캔의 안전·미처리 그룹
- 활성 Media가 2개 이상이고 최고 score 유지 결정이 저장된 유지 추천과 일치하는 그룹. 기존 스캔의 동률처럼 저장된 추천이 없으면 결정 규칙으로 유지본을 선택
- 유지할 최고 score 버전 하나를 제외한 나머지 버전을 안전하게 계획할 수 있는 그룹
- 동일 스캔의 다른 그룹과 Part 파일 경로를 공유하지 않는 그룹
- 현재 파일 처리 방식의 사전검증을 전체 통과한 그룹

항목 수 상한은 없습니다. 최고 score가 동률이면 입력 순서와 무관하게 Plex Media ID를 숫자로 비교해 가장 작은 버전 하나를 유지하고, 숫자가 아닌 ID는 대소문자를 무시한 문자열 순서로 결정합니다. 다만 안전 격리 방식은 한 번의 격리 뒤 PMS 반영·검증이 필요한 구조이므로 활성 Media가 3개 이상인 그룹을 자동 정리에서 제외합니다. `unsafe`(기본 설정의 multipart 포함), 식별정보 누락, 그룹 간 공유 경로, 모호하거나 보호할 수 없는 자막, 변경된 파일 계획처럼 자동 확정할 수 없는 그룹도 원인과 함께 제외합니다. 제외 그룹은 worker 처리 목록과 분리된 감사 행으로 저장되므로 자동 처리되지 않으며, 결과 화면을 새로고침하거나 다시 접속해도 해당 계획의 사유를 검토할 수 있습니다. 모든 그룹이 제외된 경우에도 실행 가능한 nonce를 만들지 않고 제외 검토 기록만 남깁니다.

실행 시에는 기존 선택 버전 삭제 엔진을 한 항목씩 호출합니다. 각 항목마다 Plex 연결, 서버 Machine ID, metadata, Media/Part 지문, 허용 경로와 삭제 후 상태를 다시 확인합니다. 한 항목이 실패·차단·결과 불명이 되면 같은 그룹의 아직 실행하지 않은 하위 버전만 `skipped`로 남기고, 서로 독립적인 다음 그룹은 계속 처리합니다. 중단 버튼은 현재 처리 중인 한 항목을 강제로 끊지 않고, 그 항목의 검증이 끝난 뒤 이후 항목부터 중단합니다.

선택 버전 삭제와 자동 정리는 공용 DB 전역 삭제 lease를 사용하므로 전체 FlaskFarm worker에서 한 번에 하나의 삭제 트랜잭션만 실행됩니다. 자동 정리는 시작 시 lease를 얻고 항목마다 갱신합니다. 유효한 lease는 다른 worker의 정상 작업으로 간주해 건드리지 않으며, 최소 20분의 만료 시간 뒤에도 갱신되지 않은 lease만 복구 CAS를 선점한 worker가 정리합니다. FlaskFarm 또는 플러그인을 재시작해도 진행 중이던 자동 정리를 자동 재개하지 않으며, DELETE 경계를 확정할 수 없는 항목은 `unknown`으로 남깁니다.

## 안전 격리와 외부 자막

외부 자막 처리는 기본적으로 꺼져 있습니다. 안전 격리 방식의 외부 자막 이동은 설정의 `파일 처리 방식`을 `안전 격리 (영상 + 전용 외부 자막)`로 명시적으로 선택했을 때만 동작합니다. 이 방식은 Plex Media DELETE를 호출하지 않으며 파일을 영구삭제하지 않습니다. 삭제 대상으로 확정된 단일 Part 영상과 그 영상에만 대응한다고 검증된 외부 자막을 설정한 격리 루트 아래의 작업별 폴더로 이동합니다. 격리 파일의 확인과 최종 삭제는 운영자가 별도로 수행해야 하며, 플러그인은 자동 완전삭제 기능을 제공하지 않습니다. 별도 `Plex Media DELETE + 외부 자막 정리` 방식은 아래 설명처럼 PMS가 영상을 삭제한 뒤 target-exclusive 자막을 영구 정리합니다.
...
사전확인은 함께 격리할 자막과 제외할 자막의 전체 경로 및 사유를 보여주며, 그 목록, 파일 identity, 관련 폴더 목록과 격리 루트 identity를 plan digest에 바인딩합니다. 격리 루트를 처음 검증할 때는 루트 교체를 감지하는 32-byte identity marker 파일 `.plex_dupefinder_ff-root-id`를 한 번만 생성하며, 이 파일은 미디어나 격리 대상이 아니고 기존 파일을 덮어쓰지 않습니다. 실행 직전에 동일 계획을 다시 계산하고 파일 이동 전후에도 관련 폴더 목록을 확인합니다. 경로, 목록, 파일 stat 또는 같은 stem의 sibling 영상이 하나라도 달라지면 계획에 없던 자막은 이동하지 않고 새 사전확인 또는 수동 복구를 요구합니다. 계획을 벗어나 자막을 새로 포함하거나 제외한 채 진행하지 않습니다. 결과와 작업 이력에는 실제 자막의 원본·격리 경로, 보존된 위험 예외와 사유가 기록됩니다. 경로나 귀속이 조금이라도 모호하면 처리하지 않는 fail-closed 정책입니다.

첫 파일을 이동하기 전에 immutable manifest와 Plex snapshot을 DB에 기록하고, 유지 버전에 대응하는 외부 자막은 SHA-256 검증 보호본을 먼저 만듭니다. 영상부터 격리한 다음 계획된 전용 자막만 순서대로 이동하며 각 원자 rename을 즉시 저널에 기록합니다. 유지 자막이 사라진 경우 원본 경로가 비어 있고 보호본 hash가 정확할 때만 덮어쓰기 없이 복구합니다. 경로에 다른 파일이 생겼거나 파일 내용이 달라졌으면 자동 복구하지 않고 `manual_check_required`로 잠급니다. 중간 재시작이나 부분 이동도 자동 roll-forward/rollback하지 않으며 실제 파일과 감사 상세를 확인해야 합니다.
...
관련 자막 중 symlink/reparse, 지원하지 않는 특수 파일 또는 그 밖에 snapshot을 안전하게 만들 수 없는 보호 대상이 하나라도 있으면 PMS DELETE를 보내기 전에 전체 작업을 차단합니다. 실행 직전에 Plex metadata, Media/Part fingerprint, 자막 목록·identity·SHA-256과 보호본을 다시 검증하며, 저장된 계획과 달라져도 PMS DELETE 전에 차단합니다.

서버가 최신 영상·자막 계획을 digest에 묶고 내부 확인값 `DELETE MEDIA {media_id} SUBTITLES {count} {digest12}`를 검증하면 durable `direct_delete_journal`과 보호본을 먼저 확정한 뒤 PMS Media DELETE를 한 번 보냅니다. 사용자가 이 값을 직접 입력하지는 않습니다. timeout 또는 응답 오류가 나도 DELETE를 자동 재시도하지 않습니다. 즉시 PMS를 정확히 재조회해 대상 Media가 사라졌고 유지 Media 집합과 fingerprint가 그대로인 경우에만 보호 자막 복원과 전용 자막 정리를 계속합니다. 대상 Media가 남았거나 결과를 확정할 수 없으면 전용 자막은 건드리지 않고 보호 자막만 안전하게 복원한 뒤 `unknown`/`manual_check_required`와 보호본을 남깁니다.
...
- 최신 미리보기에서 만든 plan digest와 서버 내부 확인값 검증. UI가 값을 전달하므로 사용자는 확인 문구를 입력하지 않음
...
- 선택 삭제·자동 정리 공용 전역 DB lease, 자동 정리의 항목별 갱신 및 DB 원자 선점
- 자동 정리 계획 생성·시작·각 항목 실행 시 점수와 안전 설정 snapshot 일치
- 자동 정리 계획 내 다른 그룹을 포함한 동일 Part 경로 교차 참조 없음
...
선택 버전 삭제와 자동 정리 시작에는 확인 문구 입력이나 별도의 브라우저 확인창이 없습니다. UI가 최신 미리보기를 받은 뒤 그 nonce·plan digest·서버 확인값을 즉시 실행 요청에 전달하며, 서버는 Flask session CSRF와 함께 모두 다시 검증합니다. 실행 직전 상태가 달라졌거나 nonce가 만료·재사용되면 파일 작업 전에 차단합니다.
...
스캔별 삭제 시도 횟수와 자동 정리 항목 수에는 상한이 없습니다. 결과 화면은 감사 목적으로 누적 시도 횟수를 표시합니다. 이전 버전에서 저장한 `setting_max_delete_per_run`과 `setting_batch_max_items` 값은 실행 계획에 사용하지 않습니다.
...
outbox 작업은 **작업 이력 → 삭제 후 부분 스캔 작업**에서 독립 페이지 단위로 모두 확인할 수 있습니다. 상태, 방식, section, 대상 경로, 현재/최대 시도 횟수와 결과 또는 오류가 표시되며 새로고침 버튼은 상태 조회만 수행합니다. 각 부분 스캔 Job과 삭제 감사 Action에는 DB 강제삭제 버튼이 있습니다. 이 버튼은 종료된 행을 목록에서 숨기고 ID 재사용을 막는 최소 `history_deleted` tombstone만 남기며, 파일을 삭제·복구하거나 Plex/Scanner를 다시 호출하지 않습니다. Action과 연결된 journal, 보호본·격리 복구 정보 및 배치 기록은 보존됩니다. 실행 중이거나 worker/전역 lease가 남은 작업은 강제삭제도 거부합니다.
...
- 동점이면 Plex Media ID가 가장 작은 버전 하나를 결정적으로 유지 추천
- 총점과 구성요소를 함께 저장

점수는 유지 추천의 근거입니다. 자동 정리는 안전 판정을 별도로 통과한 그룹에서 최고 score 버전 하나를 유지합니다. 동점은 Plex Media ID가 가장 작은 버전으로 결정하며, 저장된 추천과 현재 결정이 불일치하면 처리하지 않습니다.

## 데이터 저장

플러그인 전용 FlaskFarm DB에 다음 정보를 저장합니다.

- `scan_run`: 실행 상태와 서버 식별정보. 결과 삭제 뒤에는 Run ID 재사용을 막는 scrubbed `results_deleted` tombstone만 보존
- `duplicate_group`: Plex 작품 단위 중복 그룹과 안전성 판정. 결과 삭제 뒤에는 Group ID를 예약하는 scrubbed tombstone으로 보존
- `media_candidate`: Media/Part 스냅샷, 점수, 지문. 결과 삭제 뒤에는 Candidate ID를 예약하는 scrubbed tombstone으로 보존
- `action_log`: 삭제 시도, 차단, 응답 및 전후 검증 결과
- `post_delete_scan_job`: 확인된 삭제 뒤 부분 스캔을 전달하는 durable outbox 상태와 제한된 재시도 정보
- `quarantine_journal`: 실행 manifest, 파일 identity·hash, 보호본과 실제 격리 이동 경로 및 복구 상태
- `direct_delete_journal`: Plex DELETE + 외부 자막 정리 실행 manifest, PMS 결과, 자막 identity·hash, 임시 보호·복원·정리 상태 및 수동 확인 정보
- `batch_run`: 자동 정리 계획, 시작, 진행률, 중단 및 복구 상태
- `batch_item`: 자동 정리 계획에 포함된 유지·삭제 후보와 항목별 처리 결과
- `batch_exclusion`: 자동 정리에서 제외된 그룹의 공개 제목·유형·안전 사유. worker 항목과 분리되어 새로고침 뒤에도 검토 가능
- `deletion_lease`: 단건과 배치를 직렬화하는 cross-worker 전역 lease

Plex Token은 어느 테이블에도 저장하지 않습니다.

**최근 스캔 → DB 강제삭제**는 종료된 스캔의 표시용 결과 snapshot을 지우되 `scan_run`, `duplicate_group`, `media_candidate`의 숫자 ID는 각각 최소 `results_deleted` tombstone으로 예약합니다. 따라서 SQLite에서도 과거 Action/Journal의 Group·Candidate 참조가 이후 스캔의 새 행과 잘못 연결되지 않습니다. 일반 결과 API와 화면에서는 세 tombstone을 모두 숨깁니다. 종료된 `critical`/`recovery_required` journal이 연결돼 있어도 결과 snapshot만 강제 정리할 수 있지만 journal, `action_log`, `batch_run`/`batch_item`, `batch_exclusion`, `post_delete_scan_job`, 격리·보호·복구 데이터는 그대로 보존합니다. 미디어·자막 파일을 건드리거나 Plex/Plex Scanner에 요청하지 않으며, 유효한 전역 lease나 진행 중인 삭제·배치·부분 스캔이 하나라도 있으면 강제 모드도 거부합니다.

**작업 이력 → DB 강제삭제**는 종료된 Action 또는 부분 스캔 Job을 `history_deleted` tombstone으로 바꾸어 목록에서 숨기고 표시용 snapshot을 지웁니다. 숫자 ID는 재사용하지 않으며 Action과 연결된 recovery journal·보호본·배치 감사 정보는 삭제하거나 수정하지 않습니다. 이 기능도 파일/PMS를 호출하지 않고, 실행 중 상태나 live worker/전역 lease가 있으면 차단합니다.

## 제한사항

- 하나의 `plex_mate` PMS 연결만 지원합니다.
- 음악 라이브러리는 지원하지 않습니다.
- 스케줄 자동 스캔과 스캔 완료 직후 무인 자동 삭제는 포함하지 않습니다. 중복 자동 정리는 사용자가 결과 화면에서 시작한 뒤에만 추가 확인 없이 진행합니다.
...
- 자동 정리 중에는 플러그인을 동적 재로드하지 마세요. 먼저 중단을 요청하고 현재 항목이 끝난 것을 확인한 뒤 재로드하세요. unload 시 worker를 기다리지만 Plex 요청 timeout이 더 길면 현재 검증이 잠시 계속될 수 있습니다.
- 삭제 실행 요청은 플러그인 자체 CSRF·nonce·plan digest·서버 확인값으로 보호하지만, 설정 저장은 FlaskFarm 공통 `setting_save` 라우트를 사용합니다. 커스텀 FlaskFarm에서는 session cookie의 SameSite 정책과 공통 설정 저장 CSRF 정책을 확인하세요.
