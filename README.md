...
- 사용자가 시작하면 각 안전 그룹의 단독 최고 score 버전만 유지하고 모든 하위 버전을 상한 없이 순차 처리하는 중복 자동 정리
- 동점·unsafe·공유 경로·모호한 파일 계획 등 자동 확정할 수 없는 그룹은 사유와 함께 제외하고, 한 그룹이 실패해도 다른 안전 그룹은 계속 처리
...
11. 전체 정리를 원하면 결과 화면에서 `최고 점수만 남기고 자동 정리 시작`을 누릅니다. 각 안전 그룹의 단독 최고 score 버전 하나를 유지하고 모든 하위 버전을 순차 처리하며, 포함·제외 그룹과 제외 사유를 진행 화면에서 확인할 수 있습니다.
...
- 활성 Media가 2개 이상이고 score 단독 1위가 유지 추천과 일치하는 그룹
- 유지할 최고 score 버전 하나를 제외한 모든 하위 score 버전을 안전하게 계획할 수 있는 그룹
- 동일 스캔의 다른 그룹과 Part 파일 경로를 공유하지 않는 그룹
- 현재 파일 처리 방식의 사전검증을 전체 통과한 그룹

항목 수 상한은 없습니다. 다만 안전 격리 방식은 한 번의 격리 뒤 PMS 반영·검증이 필요한 구조이므로 활성 Media가 3개 이상인 그룹을 자동 정리에서 제외합니다. 최고 점수 동점, `unsafe`(기본 설정의 multipart 포함), 식별정보 누락, 그룹 간 공유 경로, 모호하거나 보호할 수 없는 자막, 변경된 파일 계획처럼 자동 확정할 수 없는 그룹도 원인과 함께 제외합니다. 제외 그룹은 worker 처리 목록과 분리된 감사 행으로 저장되므로 자동 처리되지 않으며, 결과 화면을 새로고침하거나 다시 접속해도 해당 계획의 사유를 검토할 수 있습니다. 모든 그룹이 제외된 경우에도 실행 가능한 nonce를 만들지 않고 제외 검토 기록만 남깁니다.
...
점수는 유지 추천의 근거입니다. 자동 정리는 안전 판정을 별도로 통과하고 단독 최고 score가 분명한 그룹에서만 그 버전 하나를 유지하며, 동점이나 불일치 그룹은 처리하지 않습니다.
