...
class SubtitleUiContractTests(unittest.TestCase):
    ...
    def test_quarantine_save_requires_root_and_partial_scan(self) -> None:
        ...
        self.assertIn("deleteBackend === 'quarantine' && scanMode === 'none'", self.setting)
    ...
    def test_batch_and_history_expose_subtitle_exceptions(self) -> None:
        ...
        self.assertIn("renderBatchItems(items, quarantinePlan)", self.results)
        self.assertIn(": (quarantinePlan", self.results)
    ...
    def test_history_filter_is_enforced_by_the_backend_query(self) -> None:
        self.assertIn('subtitle_filter not in ("", "excluded", "quarantined")', self.history_module)
