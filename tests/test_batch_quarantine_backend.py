...
class BatchQuarantineBackendTest(unittest.TestCase):
    def _module(self, harness):
        return sys.modules[PACKAGE_NAME + ".batch_delete_manager"]

    @staticmethod
    def _cleanup(path="C:/media/Movie.ko.srt"):
        return {
            "enabled": True,
            "backend": "quarantine",
            "status": "planned",
            "video": {"path": "C:/media/Movie.mkv", "size": 10},
            "eligible": [{"path": path, "reason": "exclusive_to_deleted_video"}],
            "excluded": [],
            "counts": {"eligible": 1, "excluded": 0, "protected": 0, "quarantined": 0},
        }

    def test_fresh_preview_requires_exact_digest_and_runtime_binding(self) -> None:
        with FlaskFarmImportHarness() as harness:
            ...
            manager = module.BatchDeleteManager(service)
            self.assertEqual(
                manager._fresh_quarantine_preview(item, journal)["plan_digest"], digest
            )
    ...
    def test_cross_item_source_collision_is_blocked_before_mutation(self) -> None:
        with FlaskFarmImportHarness() as harness:
            module = self._module(harness)
            first = {"subtitle_cleanup": self._cleanup("C:/media/shared.ko.srt")}
            second = {"subtitle_cleanup": self._cleanup("C:/media/shared.ko.srt")}
            with self.assertRaisesRegex(RuntimeError, "같은 영상 또는 자막"):
                module.BatchDeleteManager._assert_source_sets_disjoint([first, second])

    def test_persisted_quarantine_plan_never_falls_back_to_plex_delete(self) -> None:
        with FlaskFarmImportHarness() as harness:
            module = self._module(harness)
            harness.setup_module.P.ModelSetting._data.update(
                {
            ...
            item = _Record(delete_candidate_id=2)
            batch = _Record(
                id=1,
                confirmation="BATCH QUARANTINE 1 ITEMS 1 SUBTITLES 1 aaaaaaaaaaaa",
            )
            journal = _Record(plan_digest="a" * 64)
            original = module.ModelQuarantineJournal
            ...
            item.group_id = 4
            item.keep_candidate_id = 3
            item.keep_media_id = "30"
            item.delete_media_id = "20"
            batch.scan_run_id = 9
            batch.total_items = 1
            try:
                with self.assertRaisesRegex(RuntimeError, "Plex 삭제로 전환하지"):
                    manager._validate_plan_unchanged(batch)
            finally:
                module.ModelQuarantineJournal = original

    def test_worker_keeps_quarantine_item_pending_until_scan_verification(self) -> None:
        with FlaskFarmImportHarness() as harness:
            module = self._module(harness)
            harness.setup_module.P.ModelSetting._data.update(
                {
            ...
            journal = _Record(plan_digest="a" * 64)
            manager._preview_journal = lambda *args: journal
